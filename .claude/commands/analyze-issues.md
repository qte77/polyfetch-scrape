...
2. **Assess each issue** — ROI (High/Med/Low), Effort (S < 0.5d / M 1–2d / L > 2d), risk/feasibility, and dependencies. Read issue bodies with `gh issue view <N>` and the relevant code where it matters; fan out a subagent per cluster (Task tool) to keep the main context clean.
