...
- `RenderOptions(screenshots=(Screenshot(name, target), …))` → `Response.screenshots: dict[str, bytes]` — capture **multiple named** screenshots in a single `fetch()` render (each `"viewport"` or a CSS-selector element shot), taken after the waits. Removes the "one browser launch per screenshot" tax for visual-regression across an interacted state; mirrors `render_session.shot(name)` (#117). `Screenshot` is exported from the package root. Backward-compatible (singular `screenshot`/`Response.screenshot` unchanged; new fields default `()`/`{}`). Library-only — `--json` surfacing stays with #105; `full_page` and per-`after_action` interleaving intentionally out of scope (the latter is served by `render_session`). Closes #119.
- `render_session(url, *, wait_until=…, timeout=…)` — a managed, headless Patchright **session** context manager for multi-step interactive flows (act → assert → act), the interactive counterpart to single-shot `fetch(url, render=…)`. Exposes `click`/`click_text`/`fill`/`submit`/`wait_for_selector`/`wait_for_function`/`wait_ms`, `shot(name)` (viewport PNG bytes → `s.screenshots[name]`), and the managed `.page`; console + network-failure capture is always on (`s.console_errors`/`s.network_failures`), and an exception inside the block captures an `"exception"` screenshot before teardown. Chromium-only, library-only (Python-only, no CLI — structured, like `RenderOptions.actions`). Stops every consumer re-hand-rolling raw Patchright launch/teardown/capture. Reuses the #118 capture helpers (promoted `_attach_capture`/`_capture_screenshot` → `attach_capture`/`capture_screenshot` within the private `_backends` package). Closes #117.
...
- `fetch()` request-body support — new `json` (any JSON-serialisable object) and `content` (raw `bytes`) kwargs, threaded through the httpx and curl_cffi tiers so POST/PUT JSON APIs (search/pagination/GraphQL endpoints that reject bodyless requests) can be called. Backward-compatible (both default `None`). The playwright tier stays GET-only: a body request that would escalate to — or is pinned to — playwright raises `FetchError` rather than dropping the body; passing both `json` and `content` also raises. Closes #46.
- `docs/scraping-landscape.md` empirical findings — a re-probed (2026-07-06) "User-Agent string vs. client fingerprint" table (`www.hamiltoncompany.com` `403`s a browser-UA `curl` but `200`s polyfetch's `httpx`/`curl_cffi` tiers), plus two takeaway bullets: a UA swap is insufficient when a site fingerprints the client independently of the UA (the `curl_cffi`-tier rationale), and tier-1 `httpx` already clears some 403-for-raw-clients targets with reactive-only escalation. Closes #36, #39.
...
- README hero + `## What` refreshed to the shipped surface — the tagline now leads with the typed `fetch()` / reactive fallback / render depth (dropped the "opt-in e2e tests" dev-detail), and `## What` gains bullets for tier pinning/range (`tier`/`min_tier`/`max_tier`), request bodies (`json`/`content`), per-host `Throttle`, and the deeper browser tier (named screenshots, `render_session`, console/network capture). Docs-only (no API/behaviour change). Also de-duplicated the "always use `make`" tooling rule: `AGENTS.md` now references [CONTRIBUTING → Tooling discipline](CONTRIBUTING.md#tooling-discipline) instead of restating it (single source of truth). Closes #124.
