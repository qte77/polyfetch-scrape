...
| `render_options.py` | `RenderOptions` + `RenderAction` + `Screenshot` — patchright-tier controls (waits, screenshot, scripted actions, named multi-screenshots, capture_console, capture_network_failures, device (preset name **or** custom bundle)/viewport/color_scheme/user_agent/locale emulation, record_video_dir/record_video_size). |
...
- **Value objects are frozen dataclasses, not validation models.** `Response` / `RenderOptions` / `RetryPolicy` / `Throttle` are `@dataclass(frozen=True, slots=True)` — *outputs and config*, not an input-validation boundary. polyfetch therefore uses **no `pydantic` / `pydantic-settings`** and **no env/global settings** (it's a library, not an app; configuration is explicit function parameters). Untrusted input is validated where it enters: `defusedxml` / `json` parsing and the shared `utils/_ssrf.py` literal-IP guard. **Estate decision ([#164](https://github.com/qte77/polyfetch-scrape/issues/164), ratified):** polyfetch stays pydantic-free at the substrate; downstream consumers own their own config choice and may adopt `pydantic-settings` (as `analyze-stock-kpi` does) without that propagating back into core. The divergence is intentional, not accidental — a library's value objects are outputs, an app's are an input-validation boundary.
