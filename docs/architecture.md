...
| `utils/_ssrf.py` | Shared SSRF guard for the utils that fetch attacker-influenced URLs: `check_ssrf(url)` rejects a literal internal IP **and** resolves a hostname, rejecting it when *any* answer is internal; `check_redirect(url, response)` re-applies that check to the URL a response landed on and to an unfollowed 301/308 `Location`. DNS rebinding is out of scope (needs pinned-IP connect, which no tier exposes). |
